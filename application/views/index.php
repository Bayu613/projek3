...
background: linear-gradient(0deg, rgb(20,80,155) 8%, rgb(255,255,255) 10%);  box-shadow:0 0 15px 2px  rgb(194,194,194);">
	<p style="position:relative;top:15px; color:rgb(20,80,155); font-weight:600;">
		...
		<?php
    $max_items = 15; // mkaksimal item yang ditampilkan
    $count = 0;
				...
				<div class="card border-0 rounded-0 shadow ml-4 mt-4" style="width: 13.1rem;  ">
					<a><img style="width:112%; height:190px; position:relative;right:6%;" src="<?php echo base_url(). '/upload/' . $brg->gambar ?>" class="card-img-top" ></a>
						<div class="card-body mt-1 mb-3">
							<div class="row">
								<div class="">
								<?php echo anchor('detail_produk2/detail/' . $brg->produk_id, '<h8 class="card-title" style="font-size:13px;   color: rgb(85, 84, 84); text-decoration:none;">'.$brg->nama.'</h8>', 'style="text-decoration:none;"')?>
									</div>
							</div>
						</div>
						<h9 style="font-size:13px; width:100px; margin-top:10px; position:relative;top:35px; right:19px;"><small style="font-size:9px; ">Rp</small><?php echo number_format($brg->harga,0,',','.')?></h9>
						<div class="row align-items-center text-center g-0" >
					        <div class="col-4">
					        </div>
					        <div class="col-8">
							<a class="small" href="<?php echo base_url('auth/login'); ?>"><h class="btn btn-dark w-100 p-3 rounded-0 text-warning " style="font-size:10px; margin-left:0.69rem;">ADD TO CART</h></a>
					        </div>
					    </div>
					</div>
	<?php endforeach; ?>
	<div class="text-center mt-5 ">
    <a class="btn btn-dark rounded-0 " href="<?php echo base_url('index/index_all')?>" style="text-decoration:none;  color: white; position:relative;left:32.3rem; font-size:12px;">Lihat Semua</a>
</div>
	</div>
